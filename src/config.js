const config = {
    database: {
        path: './data/database.yml' // The database path.
    },
    development: {
        enabled: true, // If true, the bot will register all application commands to a specific guild (not globally).
        guildId: process.env.GUILD_ID // The guild ID to register application commands if development mode is enabled,
    },
    commands: {
    ...
    users: {
        ownerId: process.env.OWNER_ID, // The bot owner ID
        developers: process.env.DEVELOPER_IDS.split(", ") // The bot developers, remember to include your account ID with the other account IDs.
    },
